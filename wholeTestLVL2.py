...
def test_logic_level1():#Test Level 1 to check transition between Level 1 and 2. Edgar pls add you code. Helen, you could use the same transition between our levels.
    canvas_level1 = Canvas(level1, width=800, height=600)
    canvas_level1.pack(fill="both", expand=True)

    # Next Button for testing
    next_button = tk.Button(level1, text="button", command=go_to_lvl2)
    canvas_level1.create_window(400, 550, window=next_button) 
...
def changeBackground1():
    ...
    canvas_level3.itemconfig(original, image=newBackground)
    canvas_level3.image = newBackground
...
def successful_ending():
    changeBackground3() #changed from 4 to 3
    button_successend.pack_forget()
    button_unsuccessend.pack_forget()
    button_next_success.pack()

def unsuccessful_ending():
    changeBackground6()
    button_successend.pack_forget()
    button_unsuccessend.pack_forget()
    button_next_unsuccess.pack()

    # counter checks the times the button has been clicked, this will change the backgrounds along with the text
    count = 0
def next_step():
    global count
    if count == 0:
        ...
        changeBackground2()
        count += 1
        button_1.config(text="Choose Ending", command=show_choices)
        button_1.place(x=555,y=530)
    elif count == 2:  #3 was changed to 2
        button_1.pack_forget()

count2 = 0
def next_success_step():
    global count2
    if count2 == 0:
        changeBackground4() 
        count2 += 1
        button_next_success.config(text="Next", command=next_success_step)
    elif count2 == 1:
        changeBackground5()
        count2 += 1
        button_next_success.config(text="Next", command=next_success_step)  
        ...
        changeBackground8()
        count2 += 1
        button_next_success.pack_forget()

count3=0

def next_unsuccess_step():
    global count3
    if count3 == 0:
        changeBackground6()
        count3 += 1
        button_next_unsuccess.config(text="Next", command=next_unsuccess_step)
    elif count3 == 1:
        changeBackground4()
        count3 += 1
        button_next_unsuccess.config(text="Next", command=next_unsuccess_step)
        ...
        changeBackground9()
        count3 += 1
        button_next_unsuccess.pack_forget()

button_next_success = tk.Button(level3, text="Next", command=next_success_step, width=19, height=2, bg="black", fg="red", font=("courier"))
button_next_success.place(x=555, y=530)
button_next_unsuccess = tk.Button(level3, text="Next", command=next_unsuccess_step, width=19, height=2, bg="black", fg="red", font=("courier"))
button_next_unsuccess.place(x=555, y=530)

button_1 = tk.Button(level3, text="Next", command=next_step, width=19, height=2, bg="black", fg="red", font=("courier"))
button_1.place(x=555, y=530)

button_successend = tk.Button(level3, text="Confront Nightmare", command=successful_ending)
button_successend.place(x=555, y=530)
button_unsuccessend = tk.Button(level3, text="Attempt to Flee", command=unsuccessful_ending)
button_unsuccessend.place(x=555, y=530)

    #this makes the buttons that will both have different outputs appear
def show_choices():
    button_1.pack_forget()
    button_successend.pack()
    button_unsuccessend.pack()
